  ...
  const RELEASE_FEATURES = [
    ...
    },
    {
      ...
      items: [
        ...
        ['Classes AA à CC', 'Lecture des classes BULL et EXCA avec la bonne priorité : AA est le plus prioritaire, CC le plus faible.', 'undercarriage'],
...
(() => {
  ...
  function machines(row) {
    ...
    const parsed = [...keys].map(key => {
      ...
      return { key, label: key.replace(/^machine_\d+$/, 'Machine'), smr, bull, exca, travelPct, travelHours, best, score, hasData: !!(smr || bull || exca || travelPct || travelHours) };
    }).filter(m => m.hasData);
  ...
  function active() { return !!(state.enabled || state.type || state.cls || state.priority || state.smrMin || state.activityMin || state.travelPctMin || state.travelHoursMin); }
    ...
    panel.insertAdjacentHTML('beforeend', `
      <details id="wip-undercarriage-filter" class="rounded-2xl border border-orange-200 dark:border-orange-500/30 bg-orange-50/70 dark:bg-orange-500/10 p-3">
        ...
        <div class="mt-3 space-y-2 text-xs text-slate-700 dark:text-slate-200">
          ...
          <label class="wip-uc-field">Type<select data-uc="type">${opt('', 'Tous')}${opt('EXCA', 'EXCA')}${opt('BULL', 'BULL')}</select></label>
  ...
  function machines(row) {
    ...
    const list = [...keys].map((key) => {
      ...
      return {
        ...
        best,
        score,
        hasData: !!(smr || bull || exca || travelPct || travelHours)
      };
...
/* wip-grand-comptes-badge-patch.js */
(() => {
